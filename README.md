...
The simulator **cycles** the ground-truth intent in a fixed, balanced order: `left` → `right` → `up` → `down` → `rest` (one intent per 20 ms batch). That keeps spike statistics balanced across directions; `rest` uses no directional channel boost. Every 50 batches the server prints a histogram of the last 200 intents to the console.
...
- **Payload:** JSON objects with `timestamp_ms`, `predicted_intent`, `confidence`, `latency_ms`, `accuracy`, plus **`cursor_x` / `cursor_y`** in \([0,1]\) for normalized 2D cursor position (integrated per connection from predicted intent; does not change `/ws/bci-stream`). Predictions may include `rest` when the model infers a neutral pattern.
- **Notes:** this endpoint reuses the simulator stream and decodes from `spikes` (LFP is generated but not used by the decoder). The `BciDecoder` in `app/decoder.py` applies a small default **exploration** rate (~8%) so it occasionally emits an alternate intent at lower confidence (configurable via `exploration_prob`; offline tests use `0`).
